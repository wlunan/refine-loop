...
function handleEvent(data: any) { taskStore.appendTimelineEvent(data); const ts = new Date().toLocaleTimeString(); switch (data.type) {
  ...
  case 'subtask_progress': logs.value.push('['+ts+'] 第 '+data.round+' 轮完成，评分: '+data.score+'，Token: '+formatTokenCount(data.tokens_used)); if(taskStore.currentTask && typeof data.task_total_tokens==='number') taskStore.currentTask.total_tokens=data.task_total_tokens; break
  ...
  case 'task_awaiting_approval': logs.value.push('['+ts+'] 验证完成，等待确认变更'); taskStore.fetchTask(taskId); disconnect(); break
