...
import TaskTrace from '../components/task/TaskTrace.vue'
import RepairLoopPanel from '../components/task/RepairLoopPanel.vue'
...
function handleEvent(data: any) { taskStore.appendTimelineEvent(data); const payload = data.data || data; switch (data.type) {
  case 'subtask_progress': if(taskStore.currentTask && typeof payload.task_total_tokens==='number') taskStore.currentTask.total_tokens=payload.task_total_tokens; break
  case 'subtask_completed': taskStore.fetchTask(taskId); break
  case 'task_completed': case 'task_failed': case 'task_awaiting_approval': taskStore.fetchTask(taskId); disconnect(); break
} }
...
async function handleApprove() { await taskStore.approveChangeset(taskId); message.success('变更已应用到原始工作区') }
...
<template>
  <div class="tdp"><a-spin :spinning="taskStore.loading"><template v-if="taskStore.currentTask">
    ...
    <TaskTrace :task-id="taskId" :events="taskStore.timeline" /></div>
